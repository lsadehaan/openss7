...
static char const ident[] = "src/drivers/udp.c (" PACKAGE_ENVR ") " PACKAGE_DATE;
...
#ifdef HAVE_KMEMB_SKB_FRAG_T_BV_PAGE
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,24) || defined HAVE_KMEMB_SKB_FRAG_T_BV_PAGE
	tp = (typeof(tp)) skb_shinfo(skb)->frags[0].bv_page;
#else
#ifdef HAVE_KMEMB_STRUCT_SKB_FRAG_STRUCT_PAGE_P
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,24) || defined HAVE_KMEMB_SKB_FRAG_T_BV_PAGE
		skb_shinfo(skb)->frags[0].bv_page = (struct page *) tp_get(tp);
#else
#ifdef HAVE_KMEMB_STRUCT_SKB_FRAG_STRUCT_PAGE_P
 ...
 */
STATIC INLINE fastcall __hot_out int
tp_senddata(struct tp *tp, mblk_t *db, const unsigned short dport, const struct tp_options *opt,
	    mblk_t *mp)
{
	struct rtable *rt;
	int err;
...
#error HAVE_KFUNC___IP_SELECT_IDENT_2_ARGS or HAVE_KFUNC___IP_SELECT_IDENT_3_ARGS must be defined.
			...
			NF_HOOK(PF_INET, NF_IP_LOCAL_OUT, dev_net(dev), NULL, skb, NULL, dev,
				tp_ip_queue_xmit);
#elif defined HAVE_KFUNC_DST_OUTPUT
			NF_HOOK_(PF_INET, NF_IP_LOCAL_OUT, skb, NULL, dev, tp_ip_queue_xmit);
#else
	      ...
	      const t_uscalar_t DISCON_reason, mblk_t *dp)
{
	...
	if (dp != NULL) {
		err = tp_senddata(tp, NULL, tp->dport, &tp->options, dp);
	  ...
	  mblk_t *SEQ_number, struct tp *ACCEPTOR_id, const t_uscalar_t flags, mblk_t *dp)
{
	...
	case TS_WACK_CREQ:
		...
		if ((err = tp_senddata(tp, NULL, tp->dport, &tp->options, NULL))) {
...
te_unitdata_req_slow(queue_t *q, mblk_t *mp)
{
	...
	if (likely(p->DEST_length != 0)) {
		...
		if (unlikely((daddr = tp->options.ip.daddr) == INADDR_ANY))
			goto error;
		if (unlikely((dport = tp->dport) == 0))
			goto error;
	}
	err = TBADDATA;
	if (unlikely(dp == NULL))
	...
	if (unlikely(OPT_length != 0))
		if (unlikely((err = t_opts_parse_ud(OPT_buffer, OPT_length, &opts)) != 0))
			goto error;
	tp->options.ip.daddr = daddr;
	tp->dport = dport;
	if (unlikely((err = tp_senddata(tp, mp, dport, &opts, dp)) != QR_ABSORBED))
		goto error;
	return (QR_ABSORBED);
      error:
...
te_conn_req(queue_t *q, mblk_t *mp)
{
	...
	*OPT_buffer = tp->options;
	...
	if (tp_senddata(tp, mp, tp->dport, &tp->options, dp) != QR_ABSORBED) {
...
te_optdata_req(queue_t *q, mblk_t *mp)
{
	...
	if (unlikely(p->OPT_length != 0))
		if ((err = t_opts_parse(mp->b_wptr + p->OPT_offset, p->OPT_length, &opts)))
			goto error;
	if ((err = tp_senddata(tp, mp, tp->dport, &opts, dp)) != QR_ABSORBED)
		goto error;
	return (QR_ABSORBED);
      discard:
	freemsg(mp);
