...
static char const ident[] = "src/drivers/inet.c (" PACKAGE_ENVR ") " PACKAGE_DATE;
...
MODULE_ALIAS("char-major-" __stringify(CONFIG_STREAMS_CLONE_MAJOR) "-" __stringify(SS__CMAJOR_0));
MODULE_ALIAS("/dev/ip");
...
t_unitdata_req(ss_t *ss, queue_t *q, mblk_t *mp)
{
	...
	if (unlikely(ss->p.prot.type == SOCK_RAW && ss->cred.cr_uid != 0))
		...
		if ((cmsg_len = ss_cmsg_size(ss, opt, opt_len)) >= 0) {
			static unsigned char cmsg[4096];
			struct msghdr msg;
