...
static char const ident[] = "src/drivers/mtp_min.c (" PACKAGE_ENVR ") " PACKAGE_DATE;
...
struct mt {
	...
	// mtp_opt_timr_mtp_t timers; /* MTP timers */
...
static int
mtp_opt_negotiate(struct mt *mt, struct mtp_opts *ops)
{
...
mtp_check_src(struct mt *mt, mtp_addr_t * src)
{
	...
	for (sl = sl_list; sl; sl = sl->next)
		if (sl->lk.lk->ni == src->ni && sl->sp.loc->pc == src->pc)
			goto check;
	goto noaddr;
      check:
...
t_ordrel_ind(queue_t *q)
{
	...
	if ((mp = mi_allocb(q, sizeof(*p), BPRI_MED))) {
		...
		mp->b_wptr += sizeof(*p);
		if (mtp_get_state(mt) == TS_WIND_ORDREL)
			mtp_set_state(mt, TS_IDLE);
		if (mtp_get_state(mt) == TS_DATA_XFER)
			mtp_set_state(mt, TS_WREQ_ORDREL);
	     ...
	     uint dp)
{
	...
	if ((mp = mtp_build_rct(q, &msg, pvar, ni, dpc, opc, sls, dp)))
		return mtp_send_route(q, bp, &msg, mp);
	return (-ENOBUFS);
}
static inline int
	      ...
	      uint dp, uint si, caddr_t data, size_t dlen)
{
	...
	if ((mp = mtp_build_user(q, &msg, pvar, ni, dpc, opc, sls, dp, si, data, dlen)))
		return mtp_send_route(q, bp, &msg, mp);
	return (-ENOBUFS);
}
...
sl_timer_stop(struct sl *sl, const uint t)
{
	...
	case t1t:
		...
		if (sl->timers.t1t) {
			mi_strlog(sl->rq, STRLOGTE, SL_TRACE, "stopping t1t at %lu", jiffies);
			mi_timer_stop(sl->timers.t1t);
		}
		break;
	case t2t:
		if (sl->timers.t2t) {
			mi_strlog(sl->rq, STRLOGTE, SL_TRACE, "stopping t2t at %lu", jiffies);
...
sl_timer_start(struct sl *sl, const uint t)
{
	...
	case t1t:
		if (sl->timers.t1t) {
			mi_strlog(sl->rq, STRLOGTE, SL_TRACE, "starting t1t at %lu", jiffies);
			mi_timer(sl->rq, sl->timers.t1t, sl->config.t1t);
		}
		break;
	case t2t:
		if (sl->timers.t2t) {
			mi_strlog(sl->rq, STRLOGTE, SL_TRACE, "starting t2t at %lu", jiffies);
...
{
	if (msg->slc != sl->slc || msg->opc != sl->sp.adj->pc || msg->dpc != sl->sp.loc->pc)
		goto eproto;
...
static void
sl_sltm_failed(queue_t *q, struct sl *sl)
{
	/* FIXME */
}
...
sl_t1t_timeout(queue_t *q)
{
	...
	if (sl->flags & (SL_F_WACK_SLTM | SL_F_WACK_SLTM2)) {
		if (sl->flags & SL_F_WACK_SLTM) {
			mtp_send_sltm(q, NULL, SS7_PVAR_ETSI_00, 0, sl->sp.adj->pc, sl->sp.loc->pc, 0, sl->slc,
				      sl->tdata, sl->tlen);
...
sl_t2t_timeout(queue_t *q)
{
	...
	if (sl->state == SLS_INSERV) {
		...
		mtp_send_sltm(q, NULL, SS7_PVAR_ETSI_00, 0, sl->sp.adj->pc, sl->sp.loc->pc, 0, sl->slc, sl->tdata,
			      sl->tlen);
		sl_timer_start(sl, t1t);
		return (0);
	}
...
sl_recv_slta(struct sl *sl, queue_t *q, mblk_t *bp, mblk_t *dp, struct mtp_msg *msg)
{
	...
	if (sl->flags & SL_F_WACK_SLTM) {
		if ((err =
		     mtp_send_sltm(q, bp, SS7_PVAR_ETSI_00, 0, sl->sp.adj->pc, sl->sp.loc->pc, 0, sl->slc,
				   sl->tdata, sl->tlen)))
			return (err);
		sl_timer_start(sl, t1t);
		sl->flags &= ~SL_F_WACK_SLTM;
...
static int
sl_reply_upu(struct sl *sl, queue_t *q, mblk_t *bp, mblk_t *dp, struct mtp_msg *msg)
{
	__ptrace(("SSLTA Unimplemeneted\n"));
	return (-EFAULT);
}
...
mtp_dec_com(mblk_t *mp, struct mtp_msg *msg)
{
	...
	case SS7_PVAR_ANSI:
		...
		msg->arg.fsnl = (mp->b_rptr[0] >> 4) | ((mp->b_rptr[1] & 0x7) << 4);
		mp->b_rptr += 2;
		return (0);
	}
	return (-EPROTO);
...
mtp_dec_cbm(mblk_t *mp, struct mtp_msg *msg)
{
	...
	case SS7_PVAR_ANSI:
		if (mp->b_wptr < mp->b_rptr + 2)
			break;
		msg->slc = *mp->b_rptr & 0x0f;
		msg->arg.cbc = (mp->b_rptr[0] >> 4) | ((mp->b_rptr[1] & 0xf) << 4);
		mp->b_rptr += 2;
		return (0);
	}
	return (-EPROTO);
...
mtp_dec_tfc(mblk_t *mp, struct mtp_msg *msg)
{
	...
	case SS7_PVAR_ETSI:
		if (mp->b_wptr < mp->b_rptr + 2)
			break;
		msg->dest = (mp->b_rptr[0]) | ((mp->b_rptr[1] & 0x3f) << 8);
		msg->arg.stat = mp->b_rptr[1] >> 6;
		mp->b_rptr += 2;
		return (0);
	case SS7_PVAR_ANSI:
	case SS7_PVAR_JTTC:
	case SS7_PVAR_CHIN:
		if (mp->b_wptr < mp->b_rptr + 4)
			break;
		msg->dest = (mp->b_rptr[0]) | (mp->b_rptr[1] << 8) | (mp->b_rptr[2] << 16);
		msg->arg.stat = (mp->b_rptr[3] & 0x3);
		mp->b_rptr += 4;
		return (0);
	}
	return (-EPROTO);
...
mtp_dec_dlc(mblk_t *mp, struct mtp_msg *msg)
{
	...
	default:
		if (mp->b_wptr < mp->b_rptr + 2)
			break;
		msg->slc = msg->sls;
		msg->arg.sdli = (mp->b_rptr[0]) | ((mp->b_rptr[1] & 0x0f) << 8);
		mp->b_rptr += 2;
		return (0);
	case SS7_PVAR_ANSI:
		if (mp->b_wptr < mp->b_rptr + 3)
			break;
		msg->slc = (mp->b_rptr[0] & 0x0f);
		msg->arg.sdli = (mp->b_rptr[0] >> 4) | (mp->b_rptr[1] << 4) | ((mp->b_rptr[2] & 0x03) << 12);
		mp->b_rptr += 3;
		return (0);
	}
	return (-EPROTO);
...
mtp_dec_rl(mblk_t *mp, struct mtp_msg *msg)
{
	...
	case SS7_PVAR_ITUT:
		...
		if (mp->b_wptr < mp->b_rptr + 4)
			break;
		msg->dpc = (mp->b_rptr[0] | ((mp->b_rptr[1] & 0x3f) << 8));
		msg->opc = (((mp->b_rptr[1] >> 6) & 0x3) | (mp-> b_rptr[2] << 2) | ((mp->b_rptr[3] & 0x0f) << 10));
		msg->sls = (mp->b_rptr[3] >> 4) & 0x0f;
		mp->b_rptr += 4;
		return (0);
	case SS7_PVAR_ANSI:
	case SS7_PVAR_JTTC:
	case SS7_PVAR_CHIN:
		...
		if (mp->b_wptr < mp->b_rptr + 7)
			break;
		msg->dpc = ((mp->b_rptr[0] | (mp->b_rptr[1] << 8) | (mp->b_rptr[2] << 16))) & 0x00ffffff;
		msg->opc = ((mp->b_rptr[3] | (mp->b_rptr[4] << 8) | (mp->b_rptr[5] << 16))) & 0x00ffffff;
		msg->sls = mp->b_rptr[6];
		mp->b_rptr += 7;
...
sl_recv_msg(struct sl *sl, queue_t *q, mblk_t *bp, mblk_t *dp)
{
	...
	if ((err = mtp_dec_msg(q, dp, &msg)))
		goto decode;
	if (sl->lk.lk->ni != msg.ni)
		goto eproto;
	if (sl->sp.loc->pc != msg.dpc)
		goto eproto;
	if (sl->sp.adj->pc != msg.opc)
		goto eproto;
	/* message is for us, process it */
	return sl_proc_msg(sl, q, bp, dp, &msg);
      eproto:
...
mtp_data_req(queue_t *q, mblk_t *mp)
{
	...
	if (dlen == 0 || dlen > 272)
		goto baddata;
	return tp_send_msg(mt, NULL, &mt->dst, mp);
      baddata:
...
t_data_req(queue_t *q, mblk_t *mp)
{
	...
	if ((1 << mtp_get_state(mt)) & ~TSM_OUTDATA)
		goto outstate;
	if ((err = tp_send_msg(mt, NULL, &mt->dst, mp->b_cont)) == QR_ABSORBED)
		return (QR_TRIMMED);
	return (err);
      baddata:
...
t_bind_req(queue_t *q, mblk_t *mp)
{
		...
		goto badaddr;
	{
		...
		if (src->si < 3 && mt->cred.cr_uid.val != 0)
			goto acces;
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,24)
		if (src->si < 3 && !uid_eq(mt->cred.cr_uid, GLOBAL_ROOT_UID))
			goto acces;
#else
...
t_unitdata_req(queue_t *q, mblk_t *mp)
{
	...
	if (mp->b_wptr < mp->b_rptr + sizeof(*p)
		...
		if (dst->si < 3 && mt->cred.cr_uid.val != 0)
			goto acces;
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,24)
		if (dst->si < 3 && !uid_eq(mt->cred.cr_uid, GLOBAL_ROOT_UID))
			goto acces;
#else
		...
		if (!mtp_check_dst(mt, dst))
			...
			if (mtp_parse_opts(mt, &opts, mp->b_rptr + p->OPT_offset, p->OPT_length))
				goto badopt;
			if ((err = tp_send_msg(mt, &opts, dst, mp->b_cont)) == QR_ABSORBED)
				return (QR_TRIMMED);
			return (err);
		}
	}
...
t_optdata_req(queue_t *q, mblk_t *mp)
{
	...
	if ((p->DATA_flag & T_ODF_EX) || (p->DATA_flag & T_ODF_MORE))
		...
		if (mtp_parse_opts(mt, &opts, mp->b_rptr + p->OPT_offset, p->OPT_length))
			goto badopt;
		if ((err = tp_send_msg(mt, &opts, &mt->dst, mp->b_cont)) == QR_ABSORBED)
			return (QR_TRIMMED);
		return (err);
	}
      badopt:
 ...
 *  -------------------------------------------------------------------------
 */
static struct lmi_option mtp_default_option = {
 ...
 */
static struct sl *
sl_alloc_priv(queue_t *q, struct sl **slp, int index, cred_t *crp)
{
	fixme(("Unimplemented"));
	return (NULL);
}
static void
sl_free_priv(queue_t *sl)
{
	fixme(("Unimplemented"));
}
static int
mt_w_ioctl(queue_t *q, mblk_t *mp)
{
	...
	case MTP_IOC_MAGIC:
	{
		...
		case _IOC_NR(MTP_IOCGOPTION):	/* lmi_option_t */
			ret = mtp_iocgoption(q, mp);
			break;
		case _IOC_NR(MTP_IOCSOPTION):	/* lmi_option_t */
			ret = mtp_iocsoption(q, mp);
			break;
		...
		case _IOC_NR(MTP_IOCGSTATS):	/* mtp_stats_t */
			...
			break;
		case _IOC_NR(MTP_IOCCSTATS):	/* mtp_stats_t */
...
sl_r_iocack(queue_t *q, mblk_t *mp)
{
	...
	if (!mtp_lmq || !(lm = MT_PRIV(mtp_lmq)) || lm->ioc_id != iocp->ioc_id)
		goto discard;
	lm->ioc_id = 0;
	if (!(dp = mp->b_cont) || dp->b_rptr <= dp->b_datap->db_base + sizeof(*pass))
		goto discard;
	dp->b_rptr -= sizeof(*pass);
	for (mt = mtp_list; mt && mt != *(typeof(mt) *) dp->b_rptr; mt = mt->next) ;
	if (!mt || mt->ioc_id != iocp->ioc_id)
		goto discard;
	iocp->ioc_cmd = MTP_IOCCPASS;
	putnext(lm->rq, mp);
	return (QR_ABSORBED);
      discard:
	return (QR_DONE);
...
// static int mt_w_iocdata(queue_t *, mblk_t *);
/*
 *  STREAMS message handling
 *  =========================================================================
 */
...
mt_msg(queue_t *q, mblk_t *mp)
{
	...
	if (DB_TYPE(mp) == M_PROTO)
		if (mp->b_wptr >= mp->b_rptr + sizeof(uint32_t))
			if (*(uint32_t *) mp->b_rptr == T_DATA_REQ)
...
mt_qopen(queue_t *q, dev_t *devp, int oflags, int sflag, cred_t *crp)
{
	...
	*(uint *) mp->b_rptr = t1;
	...
	if (!getminor(*devp))
		sflag = CLONEOPEN;
	if ((err = mi_open_comm(&mt_opens, sizeof(*mt), q, devp, oflags, sflag, crp))) {
		freeb(mp);
		return (err);
	}
