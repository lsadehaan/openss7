 ...
 *  provides the OpenSS7 Signalling Connection Control Part Interface (SCCPI) (which is based on
 *  NPI).  The Stream, when freshly opened, is merely associated with the ASP and specific SUA-AS
 ...
 *  to describe the configuration elements associatedw with the SCTP association.  SUA-AS
 ...
 *  normal local management function that can be performed by the SCCPI interface.
 *
 *  AS are defined with the Bind and Connect process.
 *
 *  Mapping of SCCPI service primitives from the AS to the SCTP association is peformed internal to
...
#define SUA_AS_COPYRIGHT	"Copyright (c) 2008-2020  Monavacon Limited.  All Rights Reserved."
...
#define SUA_AS_BANNER		SUA_AS_DESCRIP		"\n" \
				...
				SUA_AS_DEVICE		"\n" \
				SUA_AS_CONTACT		"\n"
#define SUA_AS_SPLASH		SUA_AS_DESCRIP		" - " \
				SUA_AS_REVISION		"\n"
...
#define SUA_AS_DRV_ID		CONFIG_STREAMS_SUA_AS_MODID
#define SUA_AS_DRV_NAME		CONFIG_STREAMS_SUA_AS_NAME
#define SUA_AS_CMAJORS		CONFIG_STREAMS_SUA_AS_NMAJORS
#define SUA_AS_CMAJOR_0		CONFIG_STREAMS_SUA_AS_MAJOR
#define SUA_AS_UNITS		CONFIG_STREAMS_SUA_AS_NMINORS

#define DRV_ID		SUA_AS_DRV_ID
#define DRV_NAME	SUA_AS_DRV_NAME
#define DRV_CMAJOR	SUA_AS_CMAJOR_0
#ifdef MODULE
#define DRV_SPLASH	SUA_AS_BANNER
#else				/* MODULE */
#define DRV_SPLASH	SUA_AS_SPLASH
#endif				/* MODULE */
...
lm_i_unlink(struct up *lm, queue_t *q, mblk_t *mp)
{
	...
	if ((ioc->ioc_flag & IOC_MASK) == IOC_NONE) {
		...
		if (drv_priv(ioc->ioc_cr) != 0 && !uid_eq(ioc->ioc_cr->cr_uid, tp->cred.cr_uid))
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,5,0)
	if (lm != lm_ctrl && drv_priv(ioc->ioc_cr) != 0 && !uid_eq(ioc->ioc_cr->cr_uid, tp->cred.cr_uid))
#elif defined(HAVE_KMEMB_STRUCT_CRED_UID_VAL)
...
static struct streamtab sua_asinfo = {
	.st_rdinit = &up_rinit,
	.st_wrinit = &up_winit,
...
#endif
MODULE_PARM_DESC(modid, "Module ID for the SUA-AS driver. (0 for allocation.)");
 ...
 *  -------------------------------------------------------------------------
 */
module_init(sua_asinit);
module_exit(sua_asexit);
